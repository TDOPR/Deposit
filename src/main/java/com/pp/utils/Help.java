...
public class Help {
    ...
    public static boolean rangeInDefined(BigDecimal current, BigDecimal min, BigDecimal max){
        return Math.max(min.doubleValue(), current.doubleValue()) == Math.min(current.doubleValue(), max.doubleValue());
    }
    ...
    private static Integer countStep(Integer size, Integer num) {
        if (isNull(num))
            num = MAX_NUMBER;
        return (size + num - 1) / num;
    }

    public List<List<Object>> splitList(List<Object> list, final Integer num) {

        int limit = countStep(list.size(), num);

        List<List<Object>> splitList = Stream.iterate(0, n -> n + 1).limit(limit).parallel().map(a -> list.stream().skip(a * num).limit(num).parallel().collect(Collectors.toList())).collect(Collectors.toList());
